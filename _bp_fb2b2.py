...
print("==== around 1e1bf-1e250 ====")
for insn in md.disasm(code[0x1e140-va:0x1e250-va], ib+0x1e140):
    print(f"  {insn.address:#x}: {insn.mnemonic} {insn.op_str}")

out=bytearray(code)
bps={}
for rva in [0x1e145,0x1e247,0x1e23f,0x1e2a4]:
    bps[rva]=out[rva-va]; out[rva-va]=0xCC
pe2=bytearray(pe); pe2[rp:rp+rs]=out
bp=os.path.abspath(r"build_univ228\fb2b_bp.exe"); open(bp,"wb").write(pe2)
k32=df.k32
    ...
    if ev.dwDebugEventCode==df.EXCEPTION_DEBUG_EVENT:
        ...
        if er.ExceptionCode==0x80000003:
            ...
            if rva in bps:
                ...
                print(f"hit {rva:#x} rax={ctx.Rax:#x} rbx={ctx.Rbx:#x} rsi={ctx.Rsi:#x} rcx={ctx.Rcx:#x}")
            k32.CloseHandle(ht)
        elif er.ExceptionCode==0xC0000005:
            print("AV", hex(er.ExceptionAddress)); break
        else:
