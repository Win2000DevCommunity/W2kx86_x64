...
probes = [0x180fa, 0x18071, 0x1891a, 0x181ec]
    ...
    if code == CREATE_PROCESS_DEBUG_EVENT:
        ...
        if ecode == 0x80000003:
            if first == 0:
                ...
                print("HIT", hex(rva), "rsi", hex(ctx.Rsi), "rbx", hex(ctx.Rbx), "rcx", hex(ctx.Rcx), "rax", hex(ctx.Rax))
                if ctx.Rsi:
                    print("  [rsi]", rpm(ctx.Rsi, 16).hex(), "[rsi+34]", rpm(ctx.Rsi+0x34, 8).hex())
                ctx.Rip = addr
            ...
            print("AV", hex(fault), "rip", hex(ctx.Rip-base if base else ctx.Rip), "rsi", hex(ctx.Rsi))
