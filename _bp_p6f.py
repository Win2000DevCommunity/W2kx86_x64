...
BPS = {
    IB + 0x19298: "wcscpy",
    IB + 0x192a8: "after_cpy",
    IB + 0x192db: "wcscat",
    IB + 0x192e2: "term",
    IB + 0x192f4: "ret_parse",
    IB + 0x18ba0: "lookup",
    IB + 0x4276c: "eEcho",
}
    ...
    if de.dwDebugEventCode == df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code == 0x80000003:
            ...
            if bp is not None:
                ...
                print("HIT %s rax=%#x rcx=%#x rdx=%#x rsi=%#x rdi=%#x rbx=%#x" % (
                    BPS[bp], ctx.Rax, ctx.Rcx, ctx.Rdx, ctx.Rsi, ctx.Rdi, ctx.Rbx))
        elif code in (0xC0000005, 0xC0000374):
            ctx = df.get_thread_context(pi.hThread)
            print("EXC %#x rip=%#x av=%#x" % (code, ctx.Rip, er.ExceptionInformation[1]))
            print(" rax=%#x rcx=%#x rdx=%#x rsi=%#x rdi=%#x rbx=%#x" % (
                ctx.Rax, ctx.Rcx, ctx.Rdx, ctx.Rsi, ctx.Rdi, ctx.Rbx))
