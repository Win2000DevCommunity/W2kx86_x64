...
exe=str(Path("build_univ203/cmd_pure.exe").resolve())
cwd=str(Path("build_univ203").resolve())
...
while time.time()-t0<10:
  ...
  if de.dwDebugEventCode==CREATE_PROCESS_DEBUG_EVENT:
    ...
    if ecode==0x80000003 and base and addr==base+0x448cb:
      hits+=1
      n=C.c_size_t(); buf=(C.c_ubyte*48)()
      k32.ReadProcessMemory(pi.hProcess, C.c_void_p(base+0x5bbe2), buf, 48, C.byref(n))
      raw=bytes(buf)
      ...
      eol=C.c_uint32(); k32.ReadProcessMemory(pi.hProcess,C.c_void_p(base+0x5be04),C.byref(eol),4,C.byref(n))
      c8=C.c_uint32(); k32.ReadProcessMemory(pi.hProcess,C.c_void_p(base+0x588d8),C.byref(c8),4,C.byref(n))
      u16=[raw[i]| (raw[i+1]<<8) for i in range(0,32,2)]
      chars=''.join(chr(c) if 32<=c<127 else ('.' if c else '\\0') for c in u16)
      print("hit%d fbc8=%#x sticky=%d eol=%#x fae0=%#x c8d8=%#x rcx=%#x [%s]" % (
        hits, dw.value, stv.value, eol.value, fa.value, c8.value, ctx.Rcx & 0xffffffffffffffff, chars))
      wrote=C.c_size_t()
      ...
      print("AV first", de.u.Exception.dwFirstChance, hex(addr), "rcx", hex(ctx.Rcx))
