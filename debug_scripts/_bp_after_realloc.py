...
pe=pathlib.Path("build_univ230/cmd_fix14.exe").read_bytes()
...
print("==== after realloc ====")
for insn in md.disasm(pe[rp+(0x28b69-va):rp+(0x28c80-va)], 0x80000000+0x28b69):
    print(f"  {insn.address:#x}: {insn.mnemonic} {insn.op_str}")
    if insn.address > 0x80000000+0x28c70: break
...
IB=0x80000000
BPS={}
for rva in [0x28b7a,0x28b90,0x28bb0,0x28bd0,0x28bf0,0x28c10,0x28c40,0x28c60,0xc5ea]:
    BPS[IB+rva]=hex(rva)
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print(f"HIT {BPS[bp]} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rbx={ctx.Rbx:#x} rsi={ctx.Rsi:#x} rdi={ctx.Rdi:#x} rbp={ctx.Rbp:#x}")
            else:
                skips+=1
                if skips>8: k32.TerminateProcess(pi.hProcess,1); break
        elif code==0xC0000005:
            ctx=df.get_thread_context(pi.hThread)
            print("AV rip",hex(ctx.Rip),"rax",hex(ctx.Rax),"rcx",hex(ctx.Rcx),"rsi",hex(ctx.Rsi))
