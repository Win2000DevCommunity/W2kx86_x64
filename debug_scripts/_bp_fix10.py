...
orig={}; de=df.DEBUG_EVENT(); skips=0; entry_rsp=None
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                name=BPS[bp]
                print(f"HIT {name} rsp={ctx.Rsp:#x} rsi={ctx.Rsi:#x} rdi={ctx.Rdi:#x} rbx={ctx.Rbx:#x} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x}")
                if name=="after_3push":
                    entry_rsp=ctx.Rsp
                    st=df.read_process_mem(pi.hProcess, ctx.Rsp, 24)
                    print("  slots", [hex(struct.unpack_from("<Q",st,i)[0]) for i in range(0,24,8)])
                if name=="early_ret":
                    print("  delta", hex(ctx.Rsp - entry_rsp) if entry_rsp else None)
                    st=df.read_process_mem(pi.hProcess, ctx.Rsp, 24)
                    print("  [rsp]", [hex(struct.unpack_from("<Q",st,i)[0]) for i in range(0,24,8)])
                if name=="after_d08c" and ctx.Rsi>0x10000:
                    b=df.read_process_mem(pi.hProcess,ctx.Rsi,48)
                    if b:
                        try: print("  str", b.decode("utf-16-le","replace")[:40])
                        except: pass
            else:
                skips+=1
                if skips>5:
            ...
            ctx=df.get_thread_context(pi.hThread)
            print("HEAP",hex(ctx.Rip)); k32.TerminateProcess(pi.hProcess,1); break
