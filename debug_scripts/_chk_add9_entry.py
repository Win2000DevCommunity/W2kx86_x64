...
r=rmap[0xadd9]
print("=== add9 entry through More? / buffer check ===")
for insn in md.disasm(pe[rp+(r-va):rp+(r-va)+0x120], base+r):
    print(f"  {insn.address-base:#x}: {insn.mnemonic} {insn.op_str}")
    if insn.address>=base+r+0x100: break

# also map ae0d
print("\nae0d->", hex(rmap.get(0xae0d,-1)))
print("ae2a->", hex(rmap.get(0xae2a,-1)))
print("adf8->", hex(rmap.get(0xadf8,-1)))
