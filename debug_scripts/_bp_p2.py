...
exe=os.path.abspath("cmd_probe2.exe")
IB=0x80000000
# single-step after r9set through the call
BPS={IB+0x36268:"r9", IB+0x3627f:"call_disp", IB+0x3988e:"disp_body", IB+0x1d35c:"f4eb", IB+0x1d4f4:"f5a8", IB+0x18157:"eEcho", IB+0x19029:"pre", IB+0x489c0:"MATCH"}
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                hits+=1
                print(f"HIT {BPS[bp]} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rdx={ctx.Rdx:#x} r8={ctx.R8:#x} r9={ctx.R9:#x} rbp28={rd(ctx.Rbp+0x28,8) and hex(struct.unpack('<Q',rd(ctx.Rbp+0x28,8))[0])}")
                if hits>40: k32.TerminateProcess(pi.hProcess,1); break
        elif code in (0xC0000005,0xC0000374):
            ctx=df.get_thread_context(pi.hThread)
            print(f"EXC {code:#x} rip={ctx.Rip:#x} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rdx={ctx.Rdx:#x} r8={ctx.R8:#x} r9={ctx.R9:#x}")
            print(f"  info0={er.ExceptionInformation[0]} addr={er.ExceptionInformation[1]:#x}" if hasattr(er,'ExceptionInformation') else "")
            st=rd(ctx.Rsp,0x140)
            if st:
                n=0
                for i in range(0,0x138,8):
                    v=struct.unpack_from("<Q",st,i)[0]
                    if IB<=v<IB+0x90000 or (v>>40) in (0x7ff,0x7fff):
