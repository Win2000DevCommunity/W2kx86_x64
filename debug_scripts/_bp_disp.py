import ctypes as C, struct, sys, os
sys.path.insert(0,".")
import dbg_fault as df
k32=C.WinDLL("kernel32", use_last_error=True)
df.suppress_fault_ui()
os.chdir("build_univ230")
exe=os.path.abspath("cmd_fix20.exe")
IB=0x80000000
BPS={IB+0x17ec9:"disp_entry", IB+0x17f51:"decide", IB+0x180b7:"internal", IB+0x180a8:"external", IB+0x18157:"eEcho"}
si=df.STARTUPINFO(); si.cb=C.sizeof(si); pi=df.PROCESS_INFORMATION()
cmd=C.create_unicode_buffer(f'"{exe}" /c echo w2ktest')
assert k32.CreateProcessW(exe,cmd,None,None,False,df.DEBUG_ONLY_THIS_PROCESS,None,os.getcwd(),C.byref(si),C.byref(pi))
orig={}; de=df.DEBUG_EVENT(); skips=0; hits=0
while k32.WaitForDebugEvent(C.byref(de),25000):
    cont=df.DBG_CONTINUE
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        for a in BPS:
            b=df.read_process_mem(pi.hProcess,a,1)
            if b: orig[a]=b[0]; df.patch_byte(pi.hProcess,a,0xCC)
        if de.u.CreateProcessInfo.hFile: k32.CloseHandle(de.u.CreateProcessInfo.hFile)
    elif de.dwDebugEventCode==df.LOAD_DLL_DEBUG_EVENT:
        if de.u.LoadDll.hFile: k32.CloseHandle(de.u.LoadDll.hFile)
    elif de.dwDebugEventCode==df.EXCEPTION_DEBUG_EVENT:
        er=de.u.Exception.ExceptionRecord; code=er.ExceptionCode&0xffffffff; addr=er.ExceptionAddress
        if code==0x80000003:
            bp=addr if addr in orig else (addr-1 if addr-1 in orig else None)
            if bp is not None:
                ctx=df.get_thread_context(pi.hThread)
                df.patch_byte(pi.hProcess,bp,orig[bp]); ctx.Rip=bp; ctx.EFlags&=~0x100
                k32.SetThreadContext(pi.hThread,C.byref(ctx))
                name=BPS[bp]; hits+=1
                info=f"HIT {name} rsi={ctx.Rsi:#x} rbp={ctx.Rbp:#x}"
                a1=df.read_process_mem(pi.hProcess,ctx.Rbp+0x10,8)
                a2=df.read_process_mem(pi.hProcess,ctx.Rbp+0x18,8)
                if a1: info+=f" [rbp+0x10]={struct.unpack('<Q',a1)[0]:#x}"
                if a2: info+=f" [rbp+0x18]={struct.unpack('<Q',a2)[0]:#x}"
                if 0x10000<ctx.Rsi<0x7fffffff:
                    nd=df.read_process_mem(pi.hProcess,ctx.Rsi,0x44)
                    if nd:
                        info+=f" node[0]={struct.unpack_from('<I',nd,0)[0]:#x} node[0x34]={struct.unpack_from('<I',nd,0x34)[0]:#x} node[0x38]={struct.unpack_from('<I',nd,0x38)[0]:#x}"
                        s=struct.unpack_from('<I',nd,0x38)[0]
                        if 0x10000<s<0x7fffffff:
                            t=df.read_process_mem(pi.hProcess,s,40)
                            if t:
                                try: info+=" cmd='"+t.decode("utf-16-le","replace").split("\0")[0][:24]+"'"
                                except: pass
                print(info)
                if hits>14: k32.TerminateProcess(pi.hProcess,1); break
            else:
                skips+=1
                if skips>4: k32.TerminateProcess(pi.hProcess,1); break
        elif code in (0xC0000005,0xC0000374):
            print("EXC",hex(code)); k32.TerminateProcess(pi.hProcess,1); break
        elif code!=0x80000004: cont=df.DBG_EXCEPTION_NOT_HANDLED
    elif de.dwDebugEventCode==df.EXIT_PROCESS_DEBUG_EVENT:
        print("exit",hex(de.u.ExitProcess.dwExitCode&0xffffffff)); break
    k32.ContinueDebugEvent(de.dwProcessId,de.dwThreadId,cont)
