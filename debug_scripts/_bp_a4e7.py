...
TARGETS = {
    0x80046e88: "a4e7entry",
    0x80046ebc: "chkstk",
    0x80046ec1: "after_chk",
    0x80046f13: "call195d2",
    0x80046f1d: "after195d2",
    0x80046f5e: "calltowupper",
    0x80046f72: "cmp3f",
    0x8001389c: "bad_jne_tgt",
}
bps = {}
def setbp(addr):
    ...
    if de.dwDebugEventCode == df.EXCEPTION_DEBUG_EVENT:
        ...
        if ec == 0x80000003:
            ...
            if hit is not None:
                ...
                print(f"HIT {name} rip={hit:x} rbp={ctx.Rbp:x} rsi={ctx.Rsi:x} rax={ctx.Rax:x} rcx={ctx.Rcx:x} rdx={ctx.Rdx:x} r15={ctx.R15:x}")
                hits.append(name)
        ...
        if ec == 0xC0000005:
            ctx = CTX(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print(f"AV rip={ctx.Rip:x} rbp={ctx.Rbp:x} rsi={ctx.Rsi:x} rax={ctx.Rax:x}")
            print("hits", hits)
            break
