...
exe=str(Path("build_univ202v/cmd_pure.exe").resolve())
cwd=str(Path("build_univ202v").resolve())
...
while time.time()-t0<8:
  ...
  if de.dwDebugEventCode==CREATE_PROCESS_DEBUG_EVENT:
    ...
    if ecode==0x80000003 and base and addr==base+0x448cb:
      ...
      chars=''.join(chr(c) if 32<=c<127 else '.' for c in u16)
      print("hit%d fbc8=%#x sticky=%d fae0=%#x [%s]" % (hits, dw.value, stv.value, fa.value, chars))
      wrote=C.c_size_t()
      ...
      print("AV first", de.u.Exception.dwFirstChance, hex(addr))
