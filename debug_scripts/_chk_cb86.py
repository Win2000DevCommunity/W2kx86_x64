...
print("=== x86 around 0xcb70 ===")
fo=raw32+(0xcb70-va32)
for insn in md.disasm(src[fo:fo+0x80], base32+0xcb70):
    print(f"  {insn.address-base32:#x}: {insn.mnemonic} {insn.op_str} [{insn.bytes.hex()}]")
    if insn.address>=base32+0xcbf0: break

# also bc7d region
print("\n=== x86 around 0xbc70 ===")
fo=raw32+(0xbc70-va32)
for insn in md.disasm(src[fo:fo+0x50], base32+0xbc70):
    print(f"  {insn.address-base32:#x}: {insn.mnemonic} {insn.op_str} [{insn.bytes.hex()}]")
