...
IB=0x80000000
BPS={IB+0x19029:"pre_search", IB+0x1902e:"post_search"}
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                hits+=1
                b=df.read_process_mem(pi.hProcess,ctx.Rbp-0x210,32)
                s=b.decode("utf-16-le","replace").split("\0")[0] if b else "?"
                print(f"HIT {BPS[bp]} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rdx={ctx.Rdx:#x} r8={ctx.R8:#x} rbx={ctx.Rbx:#x} buf='{s}' raw={b[:16].hex() if b else ''}")
                if hits>6: k32.TerminateProcess(pi.hProcess,1); break
