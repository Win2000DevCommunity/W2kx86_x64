...
bps={}; de=df.DEBUG_EVENT(); t0=time.time(); base=0
def set_bp(a):
    o=read(a,1)
...
while time.time()-t0<10:
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        set_bp(base+0x14b44)  # parse enter b00c
        set_bp(base+0x14742)  # store cursor
    elif de.dwDebugEventCode==df.EXCEPTION_DEBUG_EVENT:
        er=de.u.Exception.ExceptionRecord
        if (er.ExceptionCode&0xffffffff)==0x80000003:
            ...
            if hit:
                ...
                def dump(tag):
                    print(tag)
                    for rva_g,name in [(0x6e000,"6e000"),(0x6cbc8,"cursor"),(0x6cbe0,"fbe0buf"),
                                       (0x6e840,"21840"),(0x71320,"71320"),(0x6caec,"caec")]:
                        v=u32(read(base+rva_g,4))
                        raw=read(base+rva_g,32)
                        if name in ("fbe0buf","21840","71320"):
                            try: s=raw.decode('utf-16le','replace').split('\x00')[0][:60]
                            except: s='?'
                            print(f"  {name} @{base+rva_g:#x}: {ascii(s)} hex={raw[:24].hex()}")
                        else:
                            print(f"  {name}={v:#x}")
                            if 0x10000<v<0x90000000:
                                r=read(v,32)
                                try: s=r.decode('utf-16le','replace').split('\x00')[0][:50]
                                except: s=r[:16].hex()
                                print(f"    -> {ascii(s)}")
                if rva==0x14742:
                    print("BEFORE cursor store eax", hex(ctx.Rax&0xffffffff))
                    dump("at cursor-store")
                elif rva==0x14b44:
                    dump("at parse")
                ...
                ctx.EFlags|=0x100
                C.windll.kernel32.SetThreadContext(pi.hThread,C.byref(ctx))
        elif (er.ExceptionCode&0xffffffff)==0x80000004:
