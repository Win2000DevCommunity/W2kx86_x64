...
exe=os.path.abspath("cmd_fix15.exe")
IB=0x80000000
BPS={}
for rva,n in [(0x28b7a,"flag"),(0x28bd0,"zero_path"),(0x28c15,"old_bad"),(0x28c4b,"merge"),
             (0xc5ea,"ret_c514"),(0xc622,"call_big"),(0xc627,"after_big"),(0xc678,"join")]:
    BPS[IB+rva]=n
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print(f"HIT {BPS[bp]} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rdx={ctx.Rdx:#x} rbx={ctx.Rbx:#x} rsi={ctx.Rsi:#x} rbp={ctx.Rbp:#x}")
            else:
                skips+=1
                if skips>6:
            ...
            ctx=df.get_thread_context(pi.hThread)
            print("AV",hex(ctx.Rip),hex(ctx.Rax),hex(ctx.Rcx),hex(ctx.Rbx),hex(ctx.Rsi))
