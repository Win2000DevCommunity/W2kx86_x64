...
watch = [0x3624d, 0x1e62c, 0x39892, 0x39895, 0x1d4f4, 0x1d534, 0x1d574, 0x1d5b4,
         0x1d5e7, 0x1d603, 0x1d61a, 0x1e2b4, 0x1d7f4, 0x1ea3c, 0x1d35c, 0x1e64a,
         0x45862]
for rva in watch:
    if 0 <= rva-va < len(out):
        bps[rva] = out[rva-va]
...
while len(hits) < 120:
    ...
    if code == df.EXCEPTION_DEBUG_EVENT:
        ...
        if er.ExceptionCode == 0x80000003:
            ...
            rip = ctx.Rip - 1  # int3 advances RIP
            ...
            print("RAX", hex(ctx.Rax), "RCX", hex(ctx.Rcx), "RDX", hex(ctx.Rdx), "RSI", hex(ctx.Rsi))
            print("R8", hex(ctx.R8), "R9", hex(ctx.R9), "RBP", hex(ctx.Rbp), "RSP", hex(ctx.Rsp))
            for off in range(0, 0x60, 8):
                v = df.read_u64(pi.hProcess, ctx.Rsp + off)
                rr = (v - ib) & 0xffffffff if ib <= v < ib + 0x100000 else v
                print(f"  [rsp+{off:#x}] = {v:#x} ({rr:#x})")
...
for h in hits:
    if h[0] in ("AV", "sys"):
        ...
        print(f"hit {rva:#07x} rax={rax:#x} rcx={rcx:#x} rdx={rdx:#x} r8={r8:#x} r9={r9:#x} rsi={rsi:#x}")
