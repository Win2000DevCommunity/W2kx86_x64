...
# Correct VAs: text RVA = blob+0x1000
TARGETS = {
    0x80012594: "9eba",
    0x80012699: "getcl",
    0x8001276c: "movrdi",
...
def setbp(addr):
    ...
    if not b:
        print("fail read", hex(addr)); return False
    bps[addr] = b[0]
    ok = df.patch_byte(pi.hProcess, addr, 0xCC)
    print(hex(addr), ok, "orig", hex(b[0]))
    return ok
    ...
    if de.dwDebugEventCode == df.EXCEPTION_DEBUG_EVENT:
        ...
        if ec == 0x80000003:
            ...
            if hit is not None:
                ...
                print(f"HIT {name} rbp={ctx.Rbp:x} rsi={ctx.Rsi:x} rdi={ctx.Rdi:x} r12={ctx.R12:x} rcx={ctx.Rcx:x}")
                hits.append(name)
        ...
        if ec == 0xC0000005:
            ctx = CTX(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print(f"AV rip={ctx.Rip:x} rbp={ctx.Rbp:x} rsi={ctx.Rsi:x} rdi={ctx.Rdi:x} r12={ctx.R12:x}")
            print("hits", hits)
            break
