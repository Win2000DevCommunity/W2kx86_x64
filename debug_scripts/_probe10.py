...
probes = [0x17d7e, 0x17d8e, 0x17eb0, 0x1795e]
    ...
    if code == CREATE_PROCESS_DEBUG_EVENT:
        ...
        if ecode == 0x80000003:
            if first == 0:
                ...
                print("HIT", hex(rva), "rbx", hex(ctx.Rbx), "rsi", hex(ctx.Rsi), "rdi", hex(ctx.Rdi), "rdx", hex(ctx.Rdx), "rcx", hex(ctx.Rcx))
                ctx.Rip = addr
                k32.SetThreadContext(pi.hThread, C.byref(ctx))
        elif ecode == 0xC0000005:
            ctx = CONTEXT(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print("AV rsi", hex(ctx.Rsi), "rip", hex((ctx.Rip-base)&0xffffffffffffffff))
