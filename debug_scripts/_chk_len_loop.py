...
for x86,lab in [(0xae2f,"save buf"),(0xaf7b,"or 40 loop"),(0xaf87,"len calc"),(0xaf91,"loop head"),(0xafa2,"cmp 1000"),(0xafb4,"store esi")]:
    r=rmap.get(x86)
    print(f"\n=== {lab} x86 {x86:#x} -> {r and hex(r)} ===")
    if not r: continue
    for insn in md.disasm(pe[rp+(r-va):rp+(r-va)+0x50], base+r):
        print(f"  {insn.address-base:#x}: {insn.mnemonic} {insn.op_str}")
        if insn.address>=base+r+0x40: break
