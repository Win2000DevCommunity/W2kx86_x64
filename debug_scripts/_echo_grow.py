...
for rva,n in [(0x14d19,0x100),(0x48530,0x80),(0x14e34,0x80)]:
    print('==== %#x ===='%rva)
    for insn in md.disasm(blob[rva-tr:rva-tr+n], IB+rva):
        print('  %06x: %s %s'%(insn.address-IB, insn.mnemonic, insn.op_str))

# x86 Echo grow / string too long
...
# find 'too long' xref - message push  in echo
print('==== x86 echo from ae80 (flag path) ====')
for insn in md32.disasm(xt[0xae80-xr:0xae80-xr+0x200], 0xae80):
    print('  %06x: %s %s'%(insn.address, insn.mnemonic, insn.op_str))
    if insn.address > 0xb000: break