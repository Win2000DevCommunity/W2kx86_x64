...
probes=[0x1c6b5, 0x1c77e, 0x1c783, 0x474f4, 0x474fc, 0x17c36]
de=DEBUG_EVENT(); first=0; hits=0
while True:
  ...
  if code==CREATE_PROCESS_DEBUG_EVENT:
    ...
    if ecode==0x80000003:
      if first==0: first=1
        ...
        # re-plant shared sites
        for r in probes:
          if r in orig: wpm(base+r,b'\xcc')
        if hits>30: k32.TerminateProcess(pi.hProcess,1); break
    elif ecode==0xC0000005:
      ctx=CONTEXT(); ctx.ContextFlags=CONTEXT_FULL; k32.GetThreadContext(pi.hThread,C.byref(ctx))
      print('AV rsi',hex(ctx.Rsi),'rax',hex(ctx.Rax)); k32.TerminateProcess(pi.hProcess,1); break
