...
# discover call r15 addr
bps = {}
def setbp(addr):
    b = df.read_process_mem(pi.hProcess, addr, 1)
    if not b: print("noread", hex(addr)); return False
    bps[addr] = b[0]
    ...
    if de.dwDebugEventCode == df.EXCEPTION_DEBUG_EVENT:
        ...
        if ec == 0x80000003:
            ...
            if hit is not None:
                ...
                print(f"HIT {name} rbp={ctx.Rbp:x} rax={ctx.Rax:x} rcx={ctx.Rcx:x} rdx={ctx.Rdx:x} r8={ctx.R8:x} r14={ctx.R14:x} r15={ctx.R15:x} rsp={ctx.Rsp:x}")
                top=df.read_process_mem(pi.hProcess, ctx.Rsp, 8) or b""
                if len(top)==8: print("  [rsp]", hex(struct.unpack("<Q", top)[0]))
                hits.append(name)
        ...
        if ec == 0xC0000005:
            ctx = CTX(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print(f"AV rip={ctx.Rip:x} rbp={ctx.Rbp:x} rax={ctx.Rax:x} rcx={ctx.Rcx:x} rsp={ctx.Rsp:x}")
            raw=df.read_process_mem(pi.hProcess, ctx.Rsp-8, 24) or b""
            print("stack", raw.hex())
            print("hits", hits)
            break
