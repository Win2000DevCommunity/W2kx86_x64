...
IB=0x80000000
BPS={IB+0xc622:"call_print", IB+0xc940:"print_entry", IB+0xc98a:"after_seh", IB+0xc627:"after_print"}
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print(f"HIT {BPS[bp]} rsp={ctx.Rsp:#x} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x}")
            else:
                skips+=1
                if skips>10:
                    ctx=df.get_thread_context(pi.hThread)
            ...
            ctx=df.get_thread_context(pi.hThread)
            print("AV",hex(ctx.Rip),hex(ctx.Rax),hex(ctx.Rcx),hex(ctx.Rsp))
