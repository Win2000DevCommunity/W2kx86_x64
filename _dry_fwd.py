...
pe=bytearray(pathlib.Path("build_univ56/cmd_pure.exe").read_bytes())
...
print("iat map size", len(t._iat_rva_map), "idata", hex(idata_rva))
print("sample keys", list(t._iat_rva_map.items())[:3])
print("0x1204 in map", 0x1204 in t._iat_rva_map, hex(t._iat_rva_map.get(0x1204,0)))
text_data=None; text_rva=0x1000
...
slot=t._resolve_iat_slot_va(0x4ad01204)
print("resolve", hex(slot))
vals=[t.new_base+v for v in t._iat_rva_map.values()]
print("in values", slot in vals, "rva", hex(slot-t.new_base))
...
idx=out.find(want)
print("stub", hex(tva+idx) if idx>=0 else None)
# try with dump RVAs directly as map values
n2=t._pure_rematerialize_iat_stdcall_forwarders(bytearray(pe[traw:traw+trsz]), dict(rmap), text_data, text_rva)
print("fixed with RVA map", n2)