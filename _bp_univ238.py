...
exe=os.path.abspath("cmd_pure.exe")
IB=0x80000000
BPS={IB+0x19029:"pre_search", IB+0x1902e:"post_search", IB+0x2725a:"cmp_ret", IB+0x18b5d:"lookup_ret", IB+0x18b81:"EXTERNAL", IB+0x18157:"eEcho"}
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                if bp in (IB+0x19029, IB+0x1902e):
                    b=rd(ctx.Rbp-0x210,32)
                    if b: buf=" buf="+repr(b.decode("utf-16-le","replace").split("\0")[0])
                print(f"HIT {BPS[bp]} rax={ctx.Rax:#x} rdi={ctx.Rdi:#x} rsi={ctx.Rsi:#x}{buf}")
                if hits>20: k32.TerminateProcess(pi.hProcess,1); break
