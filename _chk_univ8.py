...
t = T()
t._pure_snap_jcc_off_bare_ret_to_epilogue(blob)
t._pure_fix_arg0_loaded_from_r8_after_homes(blob)
t._pure_snap_calls_into_mov_reg_imm(blob)
t._pure_retarget_calls_to_zero_quad_helper(blob, {}, text_data, text_rva)
t._pure_fix_geparse_followup_call(blob, {}, text_data, text_rva)
...
for ch,(a,b) in chain.items():
    ...
    while True:
        ...
        k=j+1
pe[rp:rp+rs]=blob
pathlib.Path("build_univ227/cmd_univ8.exe").write_bytes(pe)
df.suppress_fault_ui()
exe=pathlib.Path("build_univ227/cmd_univ8.exe").resolve()
# also try plain subprocess for stdout
import subprocess
...
print(repr(r.stdout[:800]))
if (r.returncode&0xffffffff) not in (0,):
