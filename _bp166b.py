...
TARGETS = {
    0x80012915: "after_gph1",
    0x80012940: "after_ha1",
    0x80012962: "after_gph2",
    0x80012980: "after_ha2",
    0x800129d1: "call_gfn",
    0x80012a31: "call_wcscpy",
}
bps = {}
def setbp(addr):
    ...
    if de.dwDebugEventCode == df.EXCEPTION_DEBUG_EVENT:
        ...
        if ec == 0x80000003:
            ...
            if hit is not None:
                ...
                print(f"HIT {name} rax={ctx.Rax:x} rcx={ctx.Rcx:x} rdx={ctx.Rdx:x} r8={ctx.R8:x} rsp={ctx.Rsp:x}")
                hits.append(name)
        ...
        if ec == 0xC0000005:
            ctx = CTX(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print(f"AV rip={ctx.Rip:x} rax={ctx.Rax:x} rsp={ctx.Rsp:x}")
            print("hits", hits)
            break
