import pathlib, struct, shutil, subprocess, os, sys
src=pathlib.Path("build_univ230/cmd_fix5.exe")
dst=pathlib.Path("build_univ230/cmd_fix10.exe")
...
for prefix in (bytes.fromhex("4c89ec415d"), bytes.fromhex("498be5415d")):
    ...
    while True:
        ...
        i=j+1
print("pushimm", n_pr, "addrsp", n_asr)
dst.write_bytes(pe)

os.chdir("build_univ230")
r=subprocess.run(["cmd_fix10.exe","/c","echo","w2ktest"], capture_output=True, timeout=20)
