...
# Set SingleCommand when sticky:=1
sc=0x80058F64
n=0
for k in range(len(blob)-20):
    if blob[k:k+2]==b"\x49\xbb":
        ...
        if (v&0xFFFF)==0xBE00 and blob[k+10:k+17]==bytes.fromhex("41c70301000000"):
            store_at=k+10
            stub=bytearray()
            stub += bytes.fromhex("41c70301000000")
            stub += b"\x48\xb8"+struct.pack("<Q", sc)
            stub += b"\xc7\x00\x01\x00\x00\x00"
            ...
            n+=1
            print("SC patch at", hex(k+0x1000))
print("SC patches", n)
...
outp=pathlib.Path("build_univ257/cmd_probe_sc.exe")
