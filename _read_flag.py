...
while time.time()-t0 < 8:
    if not k32.WaitForDebugEvent(C.byref(de), 200):
        if time.time()-t0 > 3 and base:
            buf=(C.c_char*4)(); n=C.c_size_t()
            k32.ReadProcessMemory(pi.hProcess, C.c_void_p(base+0x58F64), buf, 4, C.byref(n))
            print("flag now", struct.unpack('<I', buf[:4])[0])
    ...
    if de.dwDebugEventCode==CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==EXCEPTION_BREAKPOINT and init:
            ...
            buf=(C.c_char*4)(); n2=C.c_size_t()
            k32.ReadProcessMemory(pi.hProcess, C.c_void_p(base+0x58F64), buf, 4, C.byref(n2))
            print("set+read", struct.unpack('<I', buf[:4])[0])
