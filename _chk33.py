...
print("e846", hex(rmap.get(0xE846, 0)), "e86a", hex(rmap.get(0xE86A, 0)))
md = Cs(CS_ARCH_X86, CS_MODE_64)
off = rmap[0xE846]
print("==== e846 ====")
for insn in md.disasm(text[off - text_rva : off - text_rva + 100], off):
    print(f"  {hex(insn.address)}: {insn.mnemonic} {insn.op_str}")
    if insn.address > off + 90:
        break

exe = str(pathlib.Path("build_univ33/cmd_pure.exe").resolve())
p = subprocess.run(
    [exe, "/c", "echo", "w2ktest"],
...
print("stderr", repr(p.stderr[:200]))
