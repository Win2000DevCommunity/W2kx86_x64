...
BPS = {
    0x8BD3: "call 2ce4a",
    0x2CE57: "wcscpy in helper",
    0x2CE59: "after wcscpy helper",
    0x8B19: "crt wcscpy",
}
...
def main():
    ...
    while df.k32.WaitForDebugEvent(C.byref(de), df.INFINITE):
        if de.dwDebugEventCode == df.CREATE_PROCESS_DEBUG_EVENT:
            ...
            rva = ea - (base or 0)
            if ec == 0x80000003 and base and rva in orig:
                ctx = df.get_thread_context(pi.hThread)
                label = BPS[rva]
                print(f"{label}: RAX={ctx.Rax:#x} RCX={ctx.Rcx:#x} RDX={ctx.Rdx:#x} RDI={ctx.Rdi:#x} RBP={ctx.Rbp:#x}")
                hits.append(label)
                ...
                ctx = df.get_thread_context(pi.hThread)
                print(f"AV @ 0x{rva:x} RIP={ctx.Rip:#x} RCX={ctx.Rcx:#x} RDX={ctx.Rdx:#x} hits={hits}")
                break
