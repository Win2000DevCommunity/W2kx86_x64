...
# Non-debug: launch then attach sampler via CreateProcess DEBUG after... easier: debug from start,
# ignore exceptions until N seconds then sample RIP with SuspendThread.
...
de = DEBUG_EVENT()
# Drain debug events for 3s after start, then sample by suspending
phase = "run"
while time.time() - t0 < 8:
    # Wait up to 200ms for debug event
    if k32.WaitForDebugEvent(C.byref(de), 200):
        st = DBG_CONTINUE
        if de.dwDebugEventCode == CREATE_PROCESS_DEBUG_EVENT:
            ...
            if code == EXCEPTION_BREAKPOINT and init:
                ...
                print("AV rip", hex(ctx.Rip - base), "rbp", hex(ctx.Rbp))
        ...
        if time.time() - t0 > 2.5:
            ...
            if k32.GetThreadContext(pi.hThread, C.byref(ctx)):
                ...
                if rva < 0x200000:
                    ...
                    samples[("ext", ctx.Rip)] += 1
            k32.ResumeThread(pi.hThread)
            time.sleep(0.05)

print("top RIPs:")
for rip, n in samples.most_common(15):
    print(hex(rip) if isinstance(rip, int) else rip, n)
k32.TerminateProcess(pi.hProcess, 1)
