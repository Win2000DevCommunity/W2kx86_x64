...
exe=os.path.abspath("cmd_fix6.exe")
IB=0x80000000
NAMES={
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print("HIT",NAMES[bp],"rax",hex(ctx.Rax),"rcx",hex(ctx.Rcx),"rdx",hex(ctx.Rdx),"rsi",hex(ctx.Rsi),"rdi",hex(ctx.Rdi),"rbx",hex(ctx.Rbx))
                if NAMES[bp]=="after_d08c" and ctx.Rsi>0x10000:
                    b=df.read_process_mem(pi.hProcess,ctx.Rsi,40)
                    if b:
                        try: print("  str", b.decode("utf-16-le","replace")[:30])
                        except: print("  raw", b[:40])
            else:
                skips+=1
                if skips>3:
                    ctx=df.get_thread_context(pi.hThread)
                    print("int3",hex(ctx.Rip),hex(ctx.Rcx),hex(ctx.Rax))
                    k32.TerminateProcess(pi.hProcess,1); break
        elif code==0xC0000005:
            ctx=df.get_thread_context(pi.hThread)
            print("AV",hex(ctx.Rip),"rcx",hex(ctx.Rcx),"rdx",hex(ctx.Rdx),"rax",hex(ctx.Rax),"rsi",hex(ctx.Rsi))
            ...
            ctx=df.get_thread_context(pi.hThread)
            print("HEAP",hex(ctx.Rip),hex(ctx.Rcx),hex(ctx.Rdx))
            k32.TerminateProcess(pi.hProcess,1); break
