...
exe=str(Path("build_univ206p/cmd_pure.exe").resolve())
cwd=str(Path("build_univ206p").resolve())
...
while time.time()-t0<8:
  ...
  if de.dwDebugEventCode==CREATE_PROCESS_DEBUG_EVENT:
    ...
    if ecode==0x80000003 and base and addr==base+0x448cb:
      hits+=1
      n=C.c_size_t(); buf=(C.c_ubyte*40)()
      k32.ReadProcessMemory(pi.hProcess, C.c_void_p(base+0x5bbe2), buf, 40, C.byref(n))
      ...
      u16=[buf[i]| (buf[i+1]<<8) for i in range(0,32,2)]
      chars=''.join(chr(c) if 32<=c<127 else ('.' if c else '\\0') for c in u16)
      print("hit%d fbc8=%#x sticky=%d eol=%#x fae0=%#x rax=%#x [%s]"%(hits,dw.value,stv.value,eol.value,fa.value,ctx.Rax&0xffffffff,chars))
      wrote=C.c_size_t()
