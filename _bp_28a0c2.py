...
# show rest of 28a0c after heapalloc
pe=open("build_univ230/cmd_fix13.exe","rb").read()
...
print("==== 28aa0-28b80 ====")
off=rp+(0x28aa0-va)
for insn in md.disasm(pe[off:off+0xe0], 0x80000000+0x28aa0):
...
IB=0x80000000
# BP after heapalloc restore and key points
BPS={}
for rva,name in [(0x28aa5,"after_halloc"),(0x28ac0,"p28ac0"),(0x28ae0,"p28ae0"),
                 (0x28b00,"p28b00"),(0x28b40,"p28b40"),(0x28b80,"p28b80"),
                 (0xc5ea,"ret_to_c514")]:
    BPS[IB+rva]=name
    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print(f"HIT {BPS[bp]} rip={ctx.Rip:#x} rax={ctx.Rax:#x} rcx={ctx.Rcx:#x} rbx={ctx.Rbx:#x} rsi={ctx.Rsi:#x} rdi={ctx.Rdi:#x}")
            else:
                skips+=1
                if skips>8:
                    ctx=df.get_thread_context(pi.hThread)
                    print("int3",hex(ctx.Rip)); k32.TerminateProcess(pi.hProcess,1); break
        elif code==0xC0000005:
            ctx=df.get_thread_context(pi.hThread)
            print("AV",hex(ctx.Rip),hex(ctx.Rax),hex(ctx.Rcx),hex(ctx.Rbx))
            # disasm around if in image
            if 0x80000000 <= ctx.Rip < 0x80080000:
                b=df.read_process_mem(pi.hProcess,ctx.Rip,16)
                print(" ", list(md.disasm(b or b"", ctx.Rip)))
