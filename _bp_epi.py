...
k32 = df.k32
exe = str(Path("build_univ165/cmd_pure.exe").resolve())
...
TARGETS = {
    0x80046e88: "entry",
    0x80046ec1: "after_chk",
    0x80046f1d: "after195d2",
    0x80046f9c: "epilogue",
    0x80046fa0: "addrsp",
    0x80046fa7: "ret",
}
...
for a in TARGETS: setbp(a)
de = df.DEBUG_EVENT(); t0=time.time(); entry_rsp=None
    ...
    if de.dwDebugEventCode == df.EXCEPTION_DEBUG_EVENT:
        ...
        if ec == 0x80000003:
            ...
            if hit is not None:
                ...
                if name=="entry": entry_rsp=ctx.Rsp
                delta = (ctx.Rsp - entry_rsp) if entry_rsp is not None else 0
                top = df.read_process_mem(pi.hProcess, ctx.Rsp, 16) or b""
                import struct
                q0 = struct.unpack_from("<Q", top, 0)[0] if len(top)>=8 else 0
                print(f"{name}: rsp={ctx.Rsp:x} delta={delta:+d} [rsp]={q0:x} rsi={ctx.Rsi:x}")
                k32.SetThreadContext(pi.hThread, C.byref(ctx))
            k32.ContinueDebugEvent(de.dwProcessId, de.dwThreadId, df.DBG_CONTINUE); continue
        if ec == 0xC0000005:
            ctx = CTX(); ctx.ContextFlags = CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            print(f"AV rip={ctx.Rip:x} rsp={ctx.Rsp:x}")
            break
