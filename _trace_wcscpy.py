#!/usr/bin/env python3
"""Break just before CRT sites after wcscpy to see last good state."""
import os
import ctypes as C
import pefile
...
def main():
    ...
    while df.k32.WaitForDebugEvent(C.byref(de), df.INFINITE):
        if de.dwDebugEventCode == df.CREATE_PROCESS_DEBUG_EVENT:
            ...
            rva = ea - (base or 0)
            if ec == 0x80000003 and base and base <= ea < base + 0x900000 and rva in orig:
                ctx = df.get_thread_context(pi.hThread)
                label = BPS[rva]
                print(f"{label} @ 0x{rva:x}: RAX={ctx.Rax:#x} RCX={ctx.Rcx:#x} RDX={ctx.Rdx:#x} RBP={ctx.Rbp:#x}")
                hits.append(label)
                ...
                ctx = df.get_thread_context(pi.hThread)
                print(f"AV @ 0x{rva:x} RAX={ctx.Rax:#x} hits={hits}")
                break
