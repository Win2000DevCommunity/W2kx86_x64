...
while time.time()-t0 < 20:
    ...
    if de.dwDebugEventCode==CREATE_PROCESS_DEBUG_EVENT:
        ...
        # BP on Echo return epi 0x428D7
        ctx=CONTEXT(); ctx.ContextFlags=CONTEXT_FULL
        k32.GetThreadContext(pi.hThread, C.byref(ctx))
        ctx.Dr0=base+0x428D7; ctx.Dr7=0x1
        k32.SetThreadContext(pi.hThread, C.byref(ctx))
        armed=True
        print("armed epi bp base", hex(base))
        ...
        if code==EXCEPTION_BREAKPOINT and init:
            init=False
        elif code in (EXCEPTION_SINGLE_STEP, EXCEPTION_BREAKPOINT) and armed:
            ctx=CONTEXT(); ctx.ContextFlags=CONTEXT_FULL
            k32.GetThreadContext(pi.hThread, C.byref(ctx))
            if ctx.Dr6 & 1 or ctx.Rip == base+0x428D7:
                print("Echo returning ? set /c flag")
                val=(C.c_uint32*1)(1); n=C.c_size_t()
                k32.WriteProcessMemory(pi.hProcess, C.c_void_p(base+0x58F64), val, 4, C.byref(n))
                ctx.Dr6=0; ctx.Dr7=0; ctx.EFlags|=0x10000
                k32.SetThreadContext(pi.hThread, C.byref(ctx))
                armed=False
        elif code==0xC0000005:
            addr=de.u.Exception.ExceptionRecord.ExceptionAddress or 0
            print("AV", hex(addr-base if base and addr>=base else addr)); break
    elif de.dwDebugEventCode==EXIT_PROCESS_DEBUG_EVENT:
        print("EXITED ok"); break
