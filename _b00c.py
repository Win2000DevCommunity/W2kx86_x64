...
for insn in md.disasm(xt[0xb00c-xr:0xb186-xr], 0xb00c):
    if '0x100' in insn.op_str or insn.op_str=='eax' and insn.mnemonic=='mov':
        print('%06x: %s %s'%(insn.address, insn.mnemonic, insn.op_str))
    if insn.mnemonic in ('mov','push','pop') and ('100' in insn.op_str or '40' in insn.op_str):
        print('%06x: %s %s'%(insn.address, insn.mnemonic, insn.op_str))