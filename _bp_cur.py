...
# Watch writers conceptually: break at ADAD, EFD6, and after GetCommandLine-ish
WATCH = {
    0x800146A0: "ADAD",
    0x80035A25: "EFD6",
    0x8001E68C: "FF31",
...
assert ok
bps = {}; ev = df.DEBUG_EVENT(); hProcess=None; t0=time.time(); n=0; done=False
FBC8=0x8006DBC8; C8D8=0x8006A8D8; FBE2=0x8006DBE2
...
def snap(proc, label, ctx=None):
    ...
    fb=df.read_process_mem(proc,FBE2,32) or b""
    cur=df.read_process_mem(proc,fbc8,32) or b"" if fbc8 else b""
    def u(b):
        try: return b.decode("utf-16-le","replace").split("\x00")[0][:60]
        except: return "?"
    extra=""
    if ctx: extra=f" RCX={ctx.Rcx:#x} RDX={ctx.Rdx:#x}"
    print(f"[{label}]{extra} fbc8={fbc8:#x} c8d8={c8:#x}")
    print(f"  @fbe2={u(fb)!r} @fbc8={u(cur)!r} @c8={u(df.read_process_mem(proc,c8,64) or b'')!r}")
...
while not done and time.time()-t0<12:
    ...
    if code==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        for a in WATCH: set_bp(hProcess,a)
