...
print("pushimm", t._pure_fix_push_imm_jmp_to_mov_rcx_call(blob))
print("lcall", t._pure_fix_locale_call_reg_iat_reload(blob))
...
for ch,(a,b) in chain.items():
    ...
    while True:
        ...
        k=j+1
# extend .text if blob grew
if len(blob) > rs:
    print("blob grew", len(blob)-rs)
    # may not fit in section - truncate warning
pe[rp:rp+min(len(blob),rs)] = blob[:rs]
if len(blob) > rs:
    print("WARNING truncated", len(blob)-rs)
pathlib.Path("build_univ227/cmd_univ10.exe").write_bytes(pe)
df.suppress_fault_ui()
exe=pathlib.Path("build_univ227/cmd_univ10.exe").resolve()
