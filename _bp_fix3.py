    ...
    if de.dwDebugEventCode==df.CREATE_PROCESS_DEBUG_EVENT:
        ...
        if code==0x80000003:
            ...
            if bp is not None:
                ...
                print("HIT",NAMES[bp],"rax",hex(ctx.Rax),"rdx",hex(ctx.Rdx),"rcx",hex(ctx.Rcx))
            else:
                skips+=1
                if skips>4:
                    ctx=df.get_thread_context(pi.hThread)
                    print("heap",hex(ctx.Rip),hex(ctx.Rcx),hex(ctx.Rax))
                    stk=df.read_process_mem(pi.hProcess,ctx.Rsp,0x180)
                    for i in range(0,len(stk),8):
                        q=struct.unpack_from("<Q",stk,i)[0]
                        if 0x80001000<=q<0x80050000: print(f"  {i:#x} {q:#x}")
                    k32.TerminateProcess(pi.hProcess,1); break
        elif code in (0xC0000005,0xC0000374):
            ctx=df.get_thread_context(pi.hThread)
            print("FAULT",hex(code),hex(ctx.Rip),hex(ctx.Rax))
            k32.TerminateProcess(pi.hProcess,1); break
